...
export default function AppointmentList({ status }: { status: "upcoming" | "completed" | "cancelled" }) {
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-shrink-0">
                <img
                  src={appointment.doctor.image || "/placeholder.svg?height=80&width=80"}
              ...
              <div className="flex-1">
                ...
                {appointment.notes && (
                  <p className="text-sm text-muted-foreground mb-4 border-l-2 pl-2 italic">{appointment.notes}</p>
                )}
              </div>
              ...
              <div className="flex flex-row md:flex-col gap-2 justify-end">
                ...
                {status === "completed" && (
                  <Button variant="outline" asChild>
                    <Link href={`/doctors/${appointment.doctor.id}`}>Book Again</Link>
                  </Button>
                )}
              </div>
            </div>
