...
export function DoctorFilter() {
  ...
  const handleAvailabilityChange = (availability: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      newAvailabilities = currentAvailabilities.filter((a) => a !== availability)
    }
  ...
  const handleExperienceChange = (value: number[]) => {
    ...
    updateFilters({ ...filters, minExperience: value[0], maxExperience: value[1] })
  }
              ...
              {availabilities.map((availability) => (
                <div key={availability.id} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    checked={(filters.availabilities || []).includes(availability.id)}
